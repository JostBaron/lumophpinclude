...
class tx_lumophpinclude_pi1 extends tslib_pibase {
    ...
    function updateValidUrlTableWithNewUrls($validURLs) {
        ...
        $existingURLs = $databaseConnection->exec_SELECTgetRows(
                self::URL_TABLE_FIELD_URL,
                self::URL_TABLE_NAME,
                'url IN (' . $inClause . ')'
        );
        ...
        $newURLs = array_diff($validURLs, $existingURLs);
        $newURLsWithFieldname = array();
        foreach ($newURLs as $newURL) {
            array_push(
                    $newURLsWithFieldname,
                    array(
        ...
        $databaseConnection->exec_INSERTmultipleRows(
                self::URL_TABLE_NAME,
                array(
                    self::URL_TABLE_FIELD_URL,
                ),
